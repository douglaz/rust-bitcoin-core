...
use crate::bip68::{
    RelativeLockTime, SEQUENCE_LOCKTIME_DISABLE_FLAG, SEQUENCE_LOCKTIME_MASK,
...
pub fn verify_checksequenceverify(
    ...
    debug!(
        "CSV passed: tx sequence {} >= CSV requirement {}",
        tx_masked, csv_masked
    );
    Ok(true)
}
...
mod tests {
    ...
    fn test_is_csv_trivially_satisfied() {
        ...
        assert!(is_csv_trivially_satisfied(
            SEQUENCE_LOCKTIME_DISABLE_FLAG as i64
        ));
        assert!(is_csv_trivially_satisfied(
            (SEQUENCE_LOCKTIME_DISABLE_FLAG | 100) as i64
        ));
