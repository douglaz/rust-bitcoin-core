...
pub fn validate_locktime_bip113(
    ...
    let comparison_time = if bip113_active {
        debug!("BIP113 active: using median-time-past {} instead of block time {}", 
...
mod tests {
    ...
    fn test_full_median_span() -> Result<()> {
        ...
        let headers: Vec<BlockHeader> = (0..15)
            .map(|i| create_header(1000 + i * 100))
            .collect();
    ...
    fn test_locktime_validation() -> Result<()> {
        ...
        assert!(validate_locktime_bip113(tx_locktime, block_time, mtp, true)?);
        
        // Without BIP113: use block_time (1500000100) >= tx_locktime (1500000000) = true
        assert!(validate_locktime_bip113(tx_locktime, block_time, mtp, false)?);
