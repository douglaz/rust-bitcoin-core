...
impl RelativeLockTime {
    ...
    pub fn is_satisfied(&self, input_height: u32, input_time: u32, current_height: u32, current_time: u32) -> bool {
        match self {
    ...
    for (i, input) in tx.input.iter().enumerate() {
        ...
        if sequence & SEQUENCE_LOCKTIME_DISABLE_FLAG == 0 {
            ...
            if lock_value > 0xffff {
                bail!("Invalid BIP68 sequence value in input {}: {:#x}", i, lock_value);
            }
        }
    }
...
mod tests {
    ...
    fn test_relative_lock_time_parsing() -> Result<()> {
        ...
        assert_eq!(RelativeLockTime::from_sequence(seq_disabled), RelativeLockTime::Disabled);
        
        // Test block-based lock (10 blocks)
        let seq_blocks = Sequence(10);
        assert_eq!(RelativeLockTime::from_sequence(seq_blocks), RelativeLockTime::Blocks(10));
        
        // Test time-based lock (5 * 512 seconds)
        let seq_time = Sequence(SEQUENCE_LOCKTIME_TYPE_FLAG | 5);
        assert_eq!(RelativeLockTime::from_sequence(seq_time), RelativeLockTime::Time(5));
    ...
    fn test_check_sequence_locks() -> Result<()> {
        ...
        assert!(!check_sequence_locks(&tx, &prevout_heights, &prevout_times, 105, 2000)?);
        
        // Test: block lock satisfied but time lock not
        assert!(!check_sequence_locks(&tx, &prevout_heights, &prevout_times, 110, 2000)?);
        
        // Test: both locks satisfied
        assert!(check_sequence_locks(&tx, &prevout_heights, &prevout_times, 110, 3560)?);
    ...
    fn test_version_1_transaction() -> Result<()> {
        ...
        assert!(check_sequence_locks(&tx, &prevout_heights, &prevout_times, 101, 1100)?);
