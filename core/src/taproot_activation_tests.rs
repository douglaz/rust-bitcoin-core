...
mod tests {
    ...
    fn test_activation_boundaries() {
        ...
        assert!(
            !rules.is_taproot_active(709631),
            "Block before activation should not have Taproot"
        );
        assert!(
            rules.is_taproot_active(709632),
            "First activation block should have Taproot"
        );

        // Test exact boundary conditions for SegWit
        assert!(
            !rules.is_segwit_active(481823),
            "Block before activation should not have SegWit"
        );
        assert!(
            rules.is_segwit_active(481824),
            "First activation block should have SegWit"
        );
    }
