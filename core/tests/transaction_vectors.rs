...
fn parse_tx_test(
    test: &Value,
) -> Result<(Vec<(OutPoint, TxOut)>, Transaction, ScriptFlags, String)> {
        ...
        if let Value::Array(prevout_arr) = &arr[0] {
            for prevout in prevout_arr {
                if let Value::Array(p) = prevout {
                    if p.len() >= 4 {
                        let hash_str = p[0].as_str().unwrap_or(
...
fn validate_transaction(
    ...
    for (index, input) in tx.input.iter().enumerate() {
        ...
        verify_script(&input.script_sig, &prevout.script_pubkey, flags, &checker)?;
                    ...
                    match validate_transaction(&prevouts, &tx, flags) {
                        Ok(()) => {
                            ...
                            if prevouts.len() > 0 {
                                ...
                                eprintln!(
                                    "  First prevout script: {} bytes, hex: {}",
...
fn test_debug_invalid_signature_tx() {
    ...
    if pos + script_len <= tx_bytes.len() {
        ...
        println!(
            "  ERROR: Not enough bytes for ScriptSig! Need {} but only {} remain",
            script_len,
            tx_bytes.len() - pos
        );
    }
    ...
    if pos + 4 <= tx_bytes.len() {
        ...
        println!(
            "  ERROR: Not enough bytes for sequence! Only {} bytes remain",
            tx_bytes.len() - pos
        );
    }
    ...
    if pos == tx_bytes.len() {
        ...
        let script_bytes = vec![
            ...
            0x03, 0xe1, 0x58, 0x19, 0xe2, 0xe2, 0x2f, 0x89, 0xe4, 0xb8, 0xe2, 0xf6, 0xe8, 0xf9,
            0xe4, 0xb8, 0xe2, 0xf6, 0xe8, 0xf9, 0xe4, 0xb8, 0xe2, 0xf6, 0xe8, 0xf9, 0xe4, 0xb8,
            0xe2, 0xf6, 0xe8, 0xf9, 0x00, // One more byte to make 33 total
            0xac, // OP_CHECKSIG
        ];
        ...
        assert_eq!(
            script_bytes.len(),
            35,
            "Script should be 35 bytes: 1 (push) + 33 (pubkey) + 1 (OP_CHECKSIG)"
        );

        println!("Script bytes hex: {}", hex::encode(&script_bytes));
        println!("Script length: {}", script_bytes.len());
        println!(
            "Last byte (should be 0xac for OP_CHECKSIG): 0x{:02x}",
            script_bytes[script_bytes.len() - 1]
        );
        ...
        println!(
            "ScriptSig hex: {}",
            hex::encode(tx.input[0].script_sig.as_bytes())
        );
        ...
        while pos < script_bytes.len() {
            if script_bytes[pos] <= 75 {
                ...
                if pos + 1 + push_len <= script_bytes.len() {
                    println!(
                        "    Data: {}",
                        hex::encode(&script_bytes[pos + 1..pos + 1 + push_len])
                    );
                }
                pos += 1 + push_len;
            } else {
