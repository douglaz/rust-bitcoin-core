...
fn test_bip125_rule1_explicit_signaling() -> Result<()> {
    ...
    assert!(
        !RBFPolicy::signals_rbf(&final_tx),
...
fn test_rbf_signaling_detection() -> Result<()> {
    ...
    let rbf_sequences = vec![0x00000000, 0x00000001, 0x80000000, 0xfffffffd];
    ...
    let non_rbf_sequences = vec![0xfffffffe, 0xffffffff];
    ...
    for seq in non_rbf_sequences {
        ...
        assert!(
            !RBFPolicy::signals_rbf(&tx),
            "Sequence {} should not signal RBF",
            seq
        );
    }
...
fn test_multiple_input_transaction() -> Result<()> {
    ...
    let tx1 = create_transaction(
        vec![(input1, 0, 0xfffffffd), (input2, 0, 0xfffffffd)],
