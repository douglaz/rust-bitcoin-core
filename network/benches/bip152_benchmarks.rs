...
use bitcoin::{Block, Transaction, TxOut, Amount};
...
fn bench_compact_block_creation(c: &mut Criterion) {
    ...
    for size in [10, 100, 1000, 5000].iter() {
        ...
        group.bench_with_input(
            ...
            |b, block| {
                b.iter(|| {
                    CompactBlock::from_block(black_box(block), Some(12345))
                });
            },
        );
    }
        ...
        group.bench_with_input(
            ...
            |b, (relay, compact_block)| {
                b.to_async(&rt).iter(|| async {
                    relay.process_compact_block(black_box(compact_block.clone())).await
                });
            },
        );
    }
...
fn bench_bandwidth_savings(c: &mut Criterion) {
    ...
    for size in [100, 500, 1000, 2000].iter() {
        ...
        let savings_percent = ((full_block_size - compact_block_size) as f64 / full_block_size as f64) * 100.0;
        ...
        group.bench_with_input(
            ...
            |b, block| {
                b.iter(|| {
                    bitcoin::consensus::encode::serialize(black_box(block))
                });
            },
        );
        ...
        group.bench_with_input(
            ...
            |b, compact_block| {
                b.iter(|| {
                    bitcoin::consensus::encode::serialize(black_box(compact_block))
                });
            },
        );
    }
