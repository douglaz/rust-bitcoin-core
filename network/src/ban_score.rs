...
mod tests {
    ...
    async fn test_ban_scoring() {
        ...
        let decision = manager
            .report_misbehavior(addr, Misbehavior::OrphanTransaction)
            .await;
        ...
        for _ in 0..10 {
            manager
                .report_misbehavior(addr, Misbehavior::SlowResponse)
                .await;
        }

        let decision = manager
            .report_misbehavior(addr, Misbehavior::InvalidTransaction)
            .await;
        assert!(matches!(decision, BanDecision::Disconnect));

        // Instant ban offense
        let decision = manager
            .report_misbehavior(addr, Misbehavior::InvalidBlock)
            .await;
