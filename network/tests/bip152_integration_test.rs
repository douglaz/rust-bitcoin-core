...
use bitcoin::{Amount, Block, BlockHash, Transaction, TxOut};
use network::compact_block_protocol::{CompactBlockConfig, CompactBlockProtocol, SendCmpct};
use network::compact_blocks::{BlockTxn, CompactBlock, CompactBlockRelay};
...
async fn test_bip152_full_flow() -> Result<()> {
    ...
    let messages = protocol
        .handle_cmpctblock(peer_id, compact_block.clone())
        .await?;
...
async fn test_missing_transaction_flow() -> Result<()> {
    ...
    assert!(
        ...
        "Should identify missing transactions"
    );
    ...
    assert!(
        response.is_some() || response.is_none(),
        "BlockTxn processing should complete"
    );
...
async fn test_high_bandwidth_mode() -> Result<()> {
    ...
    assert!(
        response.is_some(),
        "Should respond to high bandwidth request"
    );
